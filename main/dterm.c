...
int dterm_init_cmdtab(dterm_handle_t* dth, const char* xpath) {
    ...
    if (cmd_init(&dth->cmdtab, xpath) < 0) {
        return -2;
    }
    return 0;
}
...
void dterm_deinit(dterm_handle_t* dth) {
    ...
    // First deinitialize variables and cmdtab
    otvar_deinit(dth->vardict);
    cmd_free(dth->cmdtab);

    // Second, deinitialize the core dterm elements
    if (dth->intf != NULL) {
        dterm_close(dth);
        free(dth->intf);
    }
...
///      to fit with the more primitive birdmon dterm impl.
...
///
    ...
    ///@todo for generic dterm implementation, we need a top-level dterm 
    ///      function for user management, and this function will almost 
    ///      reference a callback into the application to resolve the user id.  
    ///      Many apps will have no users.
#   ifdef __HAS_USERS__
        birdmon_app_t* appdata = dth->ext;
        if (dth->ext != appdata) {
            fprintf(stderr, "Linkage error between dterm handle and application data.\n");
            goto dterm_prompter_TERM;
        }
#       define USERTYPE     appdata->endpoint.usertype
#   else
#       define USERTYPE     1
#   endif
            ...
            switch (cmd) {
                ...
                case ct_prompt: {
                    if (dth->intf->state == prompt_on) {
                        ...
                        dprintf(dth->fd.out, _E_MAG"%s"_E_NRM, (char*)prompt_str[USERTYPE]);
