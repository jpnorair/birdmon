...
static const cmd_t birdmon_commands[] = {
    { "bye",        &cmd_quit  },
    { "cmdls",      &cmd_cmdlist },
    { "geoloc",     &cmd_geoloc },
    { "getanow",    &cmd_getanow },
...
#           endif
    ...
    /// Last, Add commands to the cmdtab.
    for (int i=0; i<(sizeof(birdmon_commands)/sizeof(cmd_t)); i++) {
        int rc;

        rc = cmdtab_add(*init_table, birdmon_commands[i].name, (void*)birdmon_commands[i].action, (void*)EXTCMD_null);
        if (rc != 0) {
            return -1;
        }

        /// Run command with dt=NULL to initialize the command
        ///@note This is specific to birdmon, it's not a requirement of cmdtab
        birdmon_commands[i].action(NULL, NULL, NULL, NULL, 0);
    }
...
int cmd_run(const cmdtab_item_t* cmd, dterm_handle_t* dth, uint8_t* dst, int* inbytes, uint8_t* src, size_t dstmax) {
    ...
    switch ((birdmon_extcmd_t)cmd->extcmd) {
        case EXTCMD_null:
...
///@todo Consider changing dterm_handle_t* input birdmon_app_t*
