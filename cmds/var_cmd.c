...
int cmd_var(dterm_handle_t* dth, uint8_t* dst, int* inbytes, uint8_t* src, size_t dstmax) {
    ...
    *back = 0;
    ...
    if (mark == NULL) {    
        varitem = otvar_get_item(dth->vardict, (const char*)src);
        if (varitem == NULL) {
    ...
    ///5. Case of assigning a variable with a value
        ...
        *end = 0;
        ...
        if ((*mark == 0) || (strcmp(mark, "#") == 0)) {
            ...
            //otvar_del(dth->vardict, (const char*)src);
        }
        else {
            long long integer;
            double floater;
            ...
            if (*endptr == '\0') {
                otvar_add(dth->vardict, (const char*)src, VAR_Int, integer);
                goto cmd_var_END;
            }

            floater = strtod(mark, &endptr);
            if (*endptr == '\0') {
                otvar_add(dth->vardict, (const char*)src, VAR_Float, floater);
                goto cmd_var_END;
            }
            ...
            if (*mark == '\"') {
                dst[output_sz] = 0;
                otvar_add(dth->vardict, (const char*)src, VAR_String, dst);
            }
            else {
                otvar_add(dth->vardict, (const char*)src, VAR_Binary, output_sz, dst);
            }
        }
    }
